...
boxplot(b$V3~b$V1,xlab='',ylab='',col="blueviolet",border="darkblue",las=2,main=bquote(T[p]~estimation~using~codon~frequency~model~.(i)),cex=3,cex.lab=2,cex.axis=2,cex.main=3.2,ylim=c(-5,15),xaxt='n')
c=as.character(unique(sort(as.character(b$V1))))
for(nam in seq(1, length(c), by = 1)){
axis(1, at = nam,labels=substitute(''~italic(j)~'', list(j=c[nam])), las=1,cex.axis=1.5)}
...
title(xlab="Gene names",line=15,cex.lab=3)
boxplot(b$V4~b$V1,xlab='',ylab='', col="blue",border="darkblue",las=2,main=bquote(T[p2]~estimation~using~codon~frequency~model~.(i)),cex=3,cex.lab=2,cex.axis=2,cex.main=3.2,ylim=c(-5,15),xaxt='n')
c=as.character(unique(sort(as.character(b$V1))))
for(nam in seq(1, length(c), by = 1)){
axis(1, at = nam,labels=substitute(''~italic(j)~'', list(j=c[nam])), las=1,cex.axis=1.5)}
