#!/bin/bash
echo "TranscriptID,No_Transcripts,Low_quality_protein,Id,Name,Type,Description,Chromosome,ExonCount,ORF status,GC Content,GC Stretch,Missing exon,Deleted exon,Splice site change,START missing,TOGA status,Events supported in SRA,CL ratio"|sed 's/,/\t/g' > L.info.tsv
for transcriptID in `cat L.transcript_checked.lst`
do
...
echo -e "$transcriptID\t$no_transcripts\t$low_quality_protein\t$Id\t$Name\t$Type\t$Description\t$Chromosome\t$ExonCount\t$orf_val\t$GC_Content\t$GC_Stretch\t$exon_missing\t$exon_deleted\t$splice_site_change\t$start_missing\t$toga_status\t$sra_confirm" >> L.info.tsv
echo $transcriptID
done
